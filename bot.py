...
SEEN_FILE = "seen_articles.json"   # хранит хэши уже опубликованных статей
MAX_NEW_PER_RUN = 5                # максимум постов за одну итерацию
...
def save_seen(seen: set):
    # храним только последние 2000 хэшей
    data = list(seen)[-2000:]
...
def fetch_full_text(url: str) -> str:
    """Пробуем вытащить основной текст статьи."""
    try:
...
def fetch_image_url(url: str) -> str | None:
    """Извлекаем og:image из страницы статьи."""
    try:
...
def collect_articles(seen: set) -> list[dict]:
    ...
    return articles[:MAX_NEW_PER_RUN * 3]   # берём с запасом
...
**Заголовок** (цепляющий, до 80 символов, на русском языке)
...
def rewrite_article(article: dict) -> str | None:
    ...
    prompt = REWRITE_PROMPT.format(
        ...
        full_text=full_text or article["summary"],
    )
    try:
